document.addEventListener("DOMContentLoaded", function () {
    ...
    d3.csv(csvPath).then(function (csv) {
        ...
        allImages.forEach(function (imageName) {
            ...
            var shortLabel = parts.filter(part => !['sub', 'ses', 'task', 'run'].some(prefix => part.startsWith(prefix))).join('_');
        ...
        Object.keys(labelCounts).forEach(function (shortLabel) {
            ...
            checkbox.querySelector("label").addEventListener("click", function () {
                var checkboxes = document.querySelectorAll("#imageSelectionForm input[type='checkbox']");
                checkboxes.forEach(function (cb) {
                    var cbShortLabel = cb.value.split('_').filter(part => !['sub', 'ses', 'task', 'run'].some(prefix => part.startsWith(prefix))).join('_');
        ...
        searchBar.addEventListener("input", function () {
            ...
            document.querySelectorAll("#imageSelectionForm .form-check").forEach(function (checkbox) {
        ...
        document.getElementById("applySelectionButton").addEventListener("click", function () {
            ...
            selectedImageNames = Array.from(document.querySelectorAll("#imageSelectionForm input:checked")).flatMap((input) => imageMap[input.value]);
    ...
    function generatePdf() {
        ...
        selectedImageNames = Array.from(document.querySelectorAll("#imageSelectionForm input:checked")).flatMap((input) => imageMap[input.value]);
        if (selectedImageNames.length === 0) {
            notification.innerText = 'Please select images before generating the PDF.';
            return;
        }
